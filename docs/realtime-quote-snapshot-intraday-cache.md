...
指标分析页的 `1m` 和分时图优先读取该热表。`cache_only` 缺少分钟热表数据时返回空结果和 `intraday_hot_table_miss` 来源；页面按需预热或默认策略会继续按同一分钟周期回源拉取分钟 K，并写回 `stock_intraday_minute`。交易日 09:30 前的当天分时请求保持空图表，不读取当天热表也不远程回源，避免把未开始的当天曲线画满。分时/分钟周期严禁降级展示日 K。港股和美股分钟热表后续单独扩展。
...
- 当目标交易日就是当前市场日期且尚未到 09:30 时，分时图即使收到当天分钟行也只渲染空图表时间轴，等待首批真实分钟数据。
