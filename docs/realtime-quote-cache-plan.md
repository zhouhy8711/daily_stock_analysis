...
1. 后端 `StockService` 维护进程内短缓存，key 为 `bucket_start + normalized_stock_code`。`bucket_start = floor(now / REALTIME_QUOTE_CACHE_SECONDS) * REALTIME_QUOTE_CACHE_SECONDS`，表示当前缓存时间桶起点。
2. 后端只保留当前 bucket。进入新 bucket 时清空旧 quote，避免旧实时行情长期驻留。
3. `REALTIME_QUOTE_CACHE_SECONDS=0` 时禁用后端进程内实时 quote 缓存；旧名 `REALTIME_CACHE_TTL` 仅作为兼容别名读取。
4. `efinance_fetcher.py` 与 `akshare_fetcher.py` 的全市场实时 DataFrame 缓存统一读取 `get_config().realtime_quote_cache_seconds`，不再硬编码 600/1200 秒。
5. efinance/akshare 的全市场实时 DataFrame 刷新需要在缓存锁内执行；TTL 过期时并发请求只允许一个线程回源，其余线程等待后复用同一份刷新结果。
6. API 生命周期启动 `RealtimeQuoteCacheWarmer` 后台线程，默认每 1 分钟从 `stocks.index.json` 读取全量活跃 A 股代码并刷新 quote 快照。`PREFETCH_REALTIME_QUOTES=false`、`ENABLE_REALTIME_QUOTE=false` 或 `REALTIME_QUOTE_CACHE_SECONDS=0` 时跳过后台预热。
7. 默认请求路径先读最新快照，再读当前 `REALTIME_QUOTE_CACHE_SECONDS` bucket 内短缓存，最后才远程补数。
...
10. Web 系统设置页展示 `REALTIME_QUOTE_CACHE_SECONDS`，字段右侧显示当前 Python 进程内实时行情缓存占用，单位 MB。
...
| `REALTIME_QUOTE_CACHE_SECONDS` | `30` | 实时行情快照与短缓存复用秒数；按时间桶缓存单股 quote 与全市场实时 DataFrame |
| `REALTIME_QUOTE_CACHE_SECONDS=0` | - | 禁用进程内实时 quote / 全市场实时 DataFrame 命中缓存 |
...
- Web 系统设置页可编辑 `REALTIME_QUOTE_CACHE_SECONDS`，并显示实时行情缓存内存占用。
