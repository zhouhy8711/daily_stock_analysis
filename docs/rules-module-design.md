...
- 支持业务租户工作区：当前租户可看到共享规则和本租户独有规则；共享规则只读，可运行、查看和克隆；租户独有规则可编辑、删除和运行。
...
- 第一版不接入自动下单或交易执行。
- 第一版不做多用户、角色成员或权限矩阵；仍沿用当前单管理员登录体系。
- 第一版不租户化 AI 分析记录回测 `/api/v1/backtest/*`，该链路仍保持全局历史分析记录视角。

## 租户模型

升级后系统会自动创建默认租户 `default`。既有规则迁为 `visibility=shared, tenant_id=NULL`，作为共享规则库；既有规则运行和命中记录归属默认租户，避免历史实测/回测在新租户下串出。

规则有两种可见性：

- `shared`：共享规则，所有租户可见。在租户工作区中只能运行、查看和克隆，不能直接编辑或删除。
- `tenant`：租户独有规则，只对所属租户可见，可正常编辑、删除、运行和查看历史。

规则编辑和规则列表默认按当前租户工作区展示：当前租户可见共享规则和本租户独有规则，看不到其他租户独有规则。Web 通过 `X-DSA-Tenant: <tenant_key>` 传递当前租户；缺省时后端使用默认租户。

实测和 Web 规则回测的运行入口支持额外传入 `tenant_keys`。规则实测固定覆盖全部活跃租户，规则默认全选但可手动调整本次运行规则；后端收到 `mode=latest` 且未传 `tenant_keys` 的批量运行请求时，也会默认扩展为全部活跃租户。Web 规则回测保留运行租户选择：不传或单选时只运行当前/所选租户，传入多个租户或 `all/*` 时，后端会在同一次 API 启动请求内按租户拆分子运行。每个租户只执行自己可见的所选规则，`watchlist` 使用该租户自己的股票池，运行历史、同日实测去重、run reuse 和飞书通知都落在对应租户。

多租户异步运行会先做共享执行规划：规则条件逻辑相同、运行模式/日期范围/行情快照一致的任务会按逻辑指纹合并，目标股票取各租户股票池或显式股票列表的并集，只扫描一次；每个租户自己的 `stock_rule_runs` 仍单独创建，命中通过投影 link 映射回该租户的 run 和 rule。多租户批次不会复用某个单租户正在运行的 run，避免并发启动相同批次时把共享规划拆散；跨批次去重统一落到 `shared_rule_runs.execution_key`、创建者标记和进程内共享执行锁。并发看到已有 running shared run 的非创建者会先在不持有进程内锁的情况下等待数据库中的完成结果，完成后直接复用；等待超时才接管执行，避免同进程等待闭环，也减少多 worker 场景重复计算。只有某个租户独有、无法与其他租户合并的逻辑才单独扫描。这样共同逻辑的 CPU、日线预热和 live scan cache 压力从“租户数倍”降为“一份共享并集 + 少量 link 行”，租户私有逻辑仍按各自目标股票独立消耗资源。共享实测的 `shared:` live cache key 按 Web 实测 session 稳定，不随每轮行情快照变化；停止实测清理租户 cache 时会同步清理该 session 的共享 cache，避免跨快照重复预热和残留。

租户配置第一版包含：

- `stock_list_json`：租户股票池，规则 `scope=watchlist` 和 Web 实测/回测的租户子运行默认目标优先使用这里。
- `feishu_webhook_url`、`feishu_webhook_secret`、`feishu_webhook_keyword`、`feishu_max_bytes`：租户级飞书 Webhook 配置。

非飞书通知渠道第一版仍沿用全局配置；如果租户未配置飞书 Webhook，不会回退误发全局飞书 Webhook。
...
`scope=watchlist` 时，Web 页面会按运行租户范围生成股票清单：单租户时显示该租户股票池，多租户时显示所选租户股票池并集；后端在未收到显式 `stock_codes` 时会在每个租户子运行中读取该租户自己的股票池，默认租户升级时会从全局 `STOCK_LIST` 初始化。`scope=all_a_shares` 时会从前端股票索引读取所有 A 股并填入同一清单，后端在未收到显式 `stock_codes` 时也会读取同一股票索引兜底解析全量 A 股，避免后台任务以空目标运行；`scope=custom` 保留手工维护列表。股票清单支持最大化查看，并在最大化状态下按代码或名称筛选；每行股票前提供移除按钮，便于整理全量 A 股扫描范围。保存规则时仍只从清单中提取股票代码写入 `stock_codes`；运行时如果选择 watchlist，会让后端按各租户股票池解析，避免把某一个租户的股票池误用于全部租户。
...
- Web 实测固定运行全部活跃租户，规则选择器默认全选启用规则并允许手动取消部分规则；Web 规则回测运行面板支持选择“当前租户”或“全部租户”。一次多租户启动请求会生成多个租户子运行；前端聚合展示总进度和命中结果，后端按子运行所属租户分别写历史、命中、去重缓存和飞书通知。
