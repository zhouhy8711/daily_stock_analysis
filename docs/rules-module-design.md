...
- 支持启用规则的实时行情事件触发：每次获取实时行情后，对命中的启用规则立即计算、记录并推送。
...
启用状态的规则会参与实时行情事件触发。后端在 `StockService.get_realtime_quote()` 与 `StockService.get_realtime_quotes()` 成功拿到最新行情后，会将实时 quote 合成当前交易日的 daily 判断行，再按规则目标范围筛选需要计算的规则。

触发语义：

- `is_active=true` 的规则才会参与实时触发。
- `current_price`、`change_percent`、`volume_ratio`、`turnover_rate` 等实时字段直接来自实时行情。
- `open`、`high`、`low`、`close`、`pct_chg`、`volume`、`amount` 等 daily 指标会用实时 quote 合成当天判断行；均线、MACD、RSI 和历史聚合指标会基于合成后的序列重新计算。
- 命中后会写入 `stock_rule_runs` 与 `stock_rule_matches`，并通过现有通知渠道推送。
- 为避免前端 10 秒刷新或批量行情反复推送同一信号，实时触发按“规则 + 股票 + 命中交易日”去重；同一天同一规则同一股票只记录首次命中。
