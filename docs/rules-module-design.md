...
- Web 规则历史回测使用异步后台任务执行；执行中持续展示已完成股票数 / 总股票数，全部完成后再加载命中结果。日线仍先读 `stock_daily`，窗口不足时按默认数据策略补齐并写回日表。点击命中记录打开指标弹窗时使用历史 DB-only 模式，只读 `stock_daily` 和 `stock_chip_daily`，不触发实时行情、资金流、主力持仓或筹码 HTTP 请求。
