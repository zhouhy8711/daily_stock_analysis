...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **自选监控首页** - 首页默认展示 `STOCK_LIST` 自选股监控列表（未配置时回退最近历史股票），支持多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，桌面侧边栏可收拢为仅图标模式释放工作区空间；点击股票以浮窗打开报告，浮窗左侧仅展示当前股票历史；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示实时行情、历史 K 线、1/5/15/30/60 分钟K、均线、量比、关键价位、筹码峰和净特大单/净大单/净中单/净小单监控；周期切换位于 K 线图头部，后台会预热各周期以避免整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 支持联动悬浮、右键缩放和按可用数据切换指标，筹码峰随悬浮日期重算，实时分单监控每 10 秒静默刷新行情，1分钟K额外刷新分钟数据，分钟K会标识交易所时区并支持拖动时间窗口回看返回区间；真实筹码或分单源不可用时使用历史成交量和价量关系估算，保证 A 股与 BABA 等美股也能连续展示
- 🚀 **快速分析** - 通过 API 接口触发分析
