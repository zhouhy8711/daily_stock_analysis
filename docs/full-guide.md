...
| `INDICATOR_INTRADAY_REFRESH_SECONDS` | Web 指标页当日走势、核心行情与首页自选实时行情轮询周期，默认 `60`，范围 `10~3600` 秒；可在系统设置 `System` 分类中修改 | 可选 |
...
| `INDICATOR_INTRADAY_REFRESH_SECONDS` | Web 指标页当日走势、核心行情与首页自选实时行情轮询周期（秒） | `60` |
...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **自选监控首页** - 首页默认展示 `STOCK_LIST` 自选股监控列表（未配置时回退最近历史股票），支持多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，桌面侧边栏可收拢为仅图标模式释放工作区空间；点击股票以浮窗打开报告，浮窗左侧仅展示当前股票历史；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示实时行情、历史 K 线、当日走势、1/5/15/30/60 分钟K、均线、量比、市值股本、涨跌停价、涨速、委比、主力资金、关键价位，成交量标题栏集中展示量、盘后（成交量模式显示盘后量，成交额模式显示盘后额）、MA5/10 均量和换手率（历史缺失时按成交量与流通股本推导）；右侧指标区以筹码峰/实时监控 Tab 切换，默认展示筹码峰，并在筹码图下方提供筹码源返回的收盘获利、平均成本、90%/70%价格区间和集中度，实时监控切换后展示净特大单/净大单/净中单/净小单；周期切换位于 K 线图头部，后台会预热各周期以避免整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 支持联动悬浮、点击固定后用左右方向键同步移动和按可用数据切换指标，筹码峰优先展示数据源返回日期的真实逐价位分布，缺失时使用含换手率或可推导换手率日 K 的本地筹码模型并标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，三张图支持右键缩放；指标页核心行情、当日走势和首页自选列表按 `INDICATOR_INTRADAY_REFRESH_SECONDS`（默认 60 秒）静默轮询更新，“当日走势”同步刷新分钟 K、成交量与 MACD/RSI；“A股所有”Tab 保持手动加载/刷新，不每分钟请求全市场；筹码峰不再展示历史成交量或股东持仓估算值，真实逐价位筹码明细与本地模型均缺失时显示不可用，分单源不可用时实时监控仍使用价量关系估算以保持连续展示
- 🚀 **快速分析** - 通过 API 接口触发分析
...
| `/api/v1/stocks/{code}/history?period=1m` | GET | 获取 K 线数据，`period` 支持 `daily`、`1m`、`5m`、`15m`、`30m`、`60m`；A 股分钟K在东方财富不可用时回退腾讯当日分时并本地聚合，美股分钟K优先使用 CNBC 多日 OHLCV，在 YFinance 限流或 CNBC 不可用时回退 Nasdaq 当日 chart 价格序列 |
| `/api/v1/stocks/{code}/intraday-snapshot?period=1m` | GET | 获取最近有效交易日当日走势快照，返回实时 `quote`、分钟 K `data`、`refresh_interval_seconds` 与部分失败 `errors`；指标页“当日走势”使用该接口静默刷新 |
| `/api/v1/stocks/{code}/quote?freshness_seconds=60` / `/api/v1/stocks/quotes` | GET / POST | 获取单只或批量实时行情；可选 `freshness_seconds` 会压低 efinance/akshare 全市场缓存命中新鲜度，避免前端轮询被较长缓存 TTL 卡住 |
