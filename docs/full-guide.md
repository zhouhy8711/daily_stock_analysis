...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **自选监控首页** - 首页默认展示 `STOCK_LIST` 自选股监控列表（未配置时回退最近历史股票），支持多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开报告，浮窗左侧仅展示当前股票历史；报告内「指标分析」可查看实时行情、历史 K 线、成交量、均线与关键价位，相关浮窗均支持 Esc 关闭
- 🚀 **快速分析** - 通过 API 接口触发分析
