...
| `REALTIME_QUOTE_CACHE_SECONDS` | 实时行情快照与短缓存复用秒数；按时间桶缓存单股 quote 与全市场 DataFrame，`0` 表示禁用进程内缓存。旧名 `REALTIME_CACHE_TTL` 仍兼容读取 | `30` | 可选 |
...
默认文件型 SQLite 会在连接建立时启用 `WAL` 并设置 `busy_timeout`，`save_daily_data()` 也已改为按 `(code, date)` 批量原子 upsert。规则实测会先按规则所需 lookback 和股票范围一次性预热日线缓存；Web 实测使用单独的 live 数据缓存，同一次实测的所有轮次持续复用历史日线、基础筹码分布和财务事件派生缓存，当前判断日会用最新分钟热表行情合成实时 K 线并重算当日筹码分布，缺少换手率时才按最新价重估基础获利盘兜底，直到点击「停止实测」时按 session 清理，避免全 A 股扫描时按股票反复查 DB。实时行情预热优先更新进程内快照；规则实测运行期间会临时暂停后台全市场实时行情预热并跳过分钟热表归档写入，避免低优先级 quote 抓取和归档抢占实测资源。已接入 `_run_write_transaction()` 的批量写路径会在进程内串行进入 SQLite 写事务，实测进度写入遇到临时锁竞争时会跳过本次进度刷新并继续扫描，最终结果落库仍保持重试和失败显式记录。
...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **自选监控首页** - 首页默认展示 `STOCK_LIST` 自选股监控列表（未配置时回退最近历史股票），支持多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，桌面侧边栏可收拢为仅图标模式释放工作区空间；点击股票以浮窗打开报告，浮窗左侧仅展示当前股票历史；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示实时行情、历史 K 线、1/5/15/30/60 分钟K、均线、量比、市值股本、涨跌停价、涨速、委比、主力资金、关键价位，成交量标题栏集中展示量、盘后（成交量模式显示盘后量，成交额模式显示盘后额）、MA5/10 均量和换手率（历史缺失时按成交量与流通股本推导）；右侧指标区以筹码峰/实时监控 Tab 切换，默认展示筹码峰，并在筹码图下方提供筹码源返回的收盘获利、平均成本、90%/70%价格区间和集中度，实时监控切换后展示净特大单/净大单/净中单/净小单；周期切换位于 K 线图头部，后台会预热各周期以避免整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 支持联动悬浮、点击固定后用左右方向键同步移动和按可用数据切换指标，筹码峰优先展示数据源返回日期的真实逐价位分布，缺失时使用含换手率或可推导换手率日 K 的本地筹码模型并标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，三张图支持右键缩放，实时分单监控每 10 秒静默刷新行情，1分钟K额外刷新分钟数据，分钟K会标识交易所时区并支持拖动时间窗口回看返回区间；筹码峰不再展示历史成交量或股东持仓估算值，真实逐价位筹码明细与本地模型均缺失时显示不可用，分单源不可用时实时监控仍使用价量关系估算以保持连续展示
- 🚀 **快速分析** - 通过 API 接口触发分析
- 📊 **实时进度** - 分析任务状态实时更新，支持多任务并行；普通分析链路在进入 LLM 阶段后会优先尝试 LiteLLM 流式生成，并通过任务 SSE 回灌更细粒度的 `message/progress`；Web 报告、历史记录与大盘复盘相关时间统一使用 24 小时制展示
- 🧪 **规则实测** - Web 实测页复用规则回测的规则选择和股票范围，以实时行情模式在 A 股交易日 15:00 及以前周期触发；每次实测会创建仅本次实测使用的 live 数据缓存，9:30 前只把选中股票和规则所需的当天前历史日线读入该缓存，不读取分钟热表、不评估命中、不推送通知，9:30 后和午休基于同一份缓存中的历史、基础筹码分布、财务事件派生数据与已入库分钟热表运行真扫描，当前判断日会用最新分钟热表行情合成实时 K 线并重算筹码分布，点击「停止实测」时清理该缓存；规则实测和回测统一强制 `db_only`，只读取 `stock_daily`、`stock_intraday_minute`、`stock_chip_daily` 与已落库财务派生字段，缺失行情通过离线预热、收盘归档或补数据脚本补齐；从命中记录打开指标弹窗时，命中日作为红色高亮和筹码快照锚点，日 K、成交量和 MACD 会继续展示本地库中命中日之后的交易日；规则关系支持「夹板数」和「对子数」，可分别判断最新价元角分是否呈 `a.ba`（元位和分位相同且角位不同）或角分是否呈 `.bb`；趋势起涨指标支持把多头排列、均线上行、20/60 日突破、放量、MACD 多头和 MA5 偏离过滤组合为 `trend_start_signal`；财务事件指标支持“净利润断层”类条件；页面不展示回测日期筛选，命中结果按精确到秒的执行时间默认折叠分组，并在每轮分组标题展示完成时间，展开后按策略查看命中股票、执行时间和条件值，股票、行业和日期表头可切换升降序排序，每次触发日志会持续更新；15:00 后不再按旧本地行情重复扫描，新本地热表快照产生命中时会同步推送到所有已配置通知渠道；实测页「精简模式」默认开启，同一天同一规则和股票仅展示/推送首次命中，关闭后恢复逐轮累计展示和原通知去重逻辑
