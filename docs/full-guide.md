...
4. Web 规则实测页在新实时快照出现命中时，会向所有已配置通知渠道推送命中规则、股票和条件值；实测页「精简模式」默认开启，同一天同一规则和股票仅展示/推送首次命中，关闭后恢复逐轮累计展示，并沿用上一轮组合完全一致时跳过推送的旧逻辑；飞书渠道使用 `FEISHU_WEBHOOK_URL`。
...
- 🧪 **规则实测** - Web 实测页复用规则回测的规则选择和股票范围，以实时行情模式在 A 股交易日 15:00 及以前周期触发规则扫描，9:30 前和午休也允许基于已有快照/热表运行；规则关系支持「夹板数」和「对子数」，可分别判断最新价元角分是否呈 `a.ba`（元位和分位相同且角位不同）或角分是否呈 `.bb`；页面不展示回测日期筛选，命中结果按精确到秒的执行时间默认折叠分组，并在每轮分组标题展示完成时间，展开后按策略查看命中股票、执行时间和条件值，每次触发日志会持续更新；15:00 后不再按旧快照重复扫描，新实时快照产生命中时会同步推送到所有已配置通知渠道；实测页「精简模式」默认开启，同一天同一规则和股票仅展示/推送首次命中，关闭后恢复逐轮累计展示和原通知去重逻辑
