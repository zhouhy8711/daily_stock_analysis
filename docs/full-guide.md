...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **自选监控首页** - 首页默认展示 `STOCK_LIST` 自选股监控列表（未配置时回退最近历史股票），支持多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，桌面侧边栏可收拢为仅图标模式释放工作区空间；点击股票以浮窗打开报告，浮窗左侧仅展示当前股票历史；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示实时行情、历史 K 线、1/5/15/30/60 分钟K、均线、量比、关键价位；右侧指标区以筹码峰/实时监控 Tab 切换，默认展示筹码峰，并在筹码图下方提供筹码源返回的收盘获利、平均成本、90%/70%价格区间和集中度，实时监控切换后展示净特大单/净大单/净中单/净小单；周期切换位于 K 线图头部，后台会预热各周期以避免整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 支持联动悬浮、点击固定后用左右方向键同步移动和按可用数据切换指标，筹码峰优先展示数据源返回日期的真实逐价位分布，缺失时使用含换手率或可推导换手率日 K 的本地筹码模型并标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，三张图支持右键缩放，实时分单监控每 10 秒静默刷新行情，1分钟K额外刷新分钟数据，分钟K会标识交易所时区并支持拖动时间窗口回看返回区间；筹码峰不再展示历史成交量或股东持仓估算值，真实逐价位筹码明细与本地模型均缺失时显示不可用，分单源不可用时实时监控仍使用价量关系估算以保持连续展示
- 🚀 **快速分析** - 通过 API 接口触发分析
