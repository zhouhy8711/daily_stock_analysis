...
- 🧪 **规则实测** - Web 实测页复用规则回测的规则选择和股票范围，以实时行情模式在 A 股交易时段周期触发规则扫描；页面不展示回测日期筛选，命中结果按精确到秒的执行时间默认折叠分组，并在每轮分组标题展示完成时间，展开后按策略查看命中股票、执行时间和条件值，每次触发日志会持续更新；休市后不再按旧快照重复扫描，新实时快照产生命中时会同步推送到所有已配置通知渠道
