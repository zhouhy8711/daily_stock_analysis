...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **自选监控首页** - 首页默认展示 `STOCK_LIST` 自选股监控列表（未配置时回退最近历史股票），支持多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开报告，浮窗左侧仅展示当前股票历史；报告内「指标分析」可查看实时行情、历史 K 线、1/5/15/30/60 分钟K、成交量、均线与关键价位，1分钟K打开时每 10 秒静默刷新行情与分钟数据，分钟K会标识交易所时区并支持拖动时间窗口回看返回区间；主力持仓与筹码分布默认折叠，展开后以左侧趋势图展示收盘价、筹码成本、主力动能与获利盘，支持悬停明细，并可在图表右上角按主力名称下拉切换联动，右侧将随主力变化的主力指标与市场口径基础指标分栏展示；K 线图区域保持原有图表、趋势摘要和数据说明内容，并支持同风格折叠；真实筹码源不可用时使用历史成交量估算成本分布，相关浮窗均支持 Esc 关闭
- 🚀 **快速分析** - 通过 API 接口触发分析
