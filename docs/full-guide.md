...
- 🧪 **规则实测** - Web 实测页复用规则回测的规则选择和股票范围，以实时行情模式在 A 股交易日 15:00 及以前周期触发；每次实测会创建仅本次实测使用的 live 数据缓存，9:30 前只把选中股票和规则所需的当天前历史日线读入该缓存，不读取分钟热表、不评估命中、不推送通知，9:30 后和午休基于同一份缓存中的历史、基础筹码分布、财务事件派生数据与已入库分钟热表运行真扫描，当前判断日会用最新分钟热表行情合成实时 K 线并重算筹码分布，点击「停止实测」时清理该缓存；规则实测和回测统一强制 `db_only`，只读取 `stock_daily`、`stock_intraday_minute`、`stock_chip_daily` 与已落库财务派生字段，缺失行情通过离线预热、收盘归档或补数据脚本补齐；规则关系支持「夹板数」和「对子数」，可分别判断最新价元角分是否呈 `a.ba`（元位和分位相同且角位不同）或角分是否呈 `.bb`；财务事件指标支持“净利润断层”类条件；页面不展示回测日期筛选，命中结果按精确到秒的执行时间默认折叠分组，并在每轮分组标题展示完成时间，展开后按策略查看命中股票、执行时间和条件值，每次触发日志会持续更新；15:00 后不再按旧本地行情重复扫描，新本地热表快照产生命中时会同步推送到所有已配置通知渠道；实测页「精简模式」默认开启，同一天同一规则和股票仅展示/推送首次命中，关闭后恢复逐轮累计展示和原通知去重逻辑
