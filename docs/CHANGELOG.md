...
- [改进] Web 指标分析新增默认折叠的主力持仓与筹码分布面板，展开后以左侧趋势图和右侧核心指标展示，支持在图表右上角按主力名称下拉切换联动，默认展示所有主力合计；真实筹码源不可用时使用历史成交量估算成本分布
...
- [改进] Web 指标分析页将周期切换移入 K 线图头部，K线、成交量、MACD 支持联动悬浮与右键缩放，筹码峰随悬浮日期重算，实时分单监控每 10 秒静默刷新行情
