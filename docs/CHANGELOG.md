...
- [新功能] Web 指标分析页新增“当日走势”周期，按最近有效交易日分钟 K 展示 K线、成交量、MACD/RSI，并通过 `/api/v1/stocks/{code}/intraday-snapshot` 静默刷新
- [改进] 首页自选列表与指标页核心行情接入统一实时行情状态，按 `INDICATOR_INTRADAY_REFRESH_SECONDS` 默认 60 秒轮询更新；“A股所有”仍保持手动批量加载，避免每分钟请求全市场
- [改进] 实时行情 `/quote` 与 `/quotes` 支持 `freshness_seconds`，efinance/akshare 全市场缓存命中会按请求新鲜度压低有效 TTL
- [文档] 补充 `INDICATOR_INTRADAY_REFRESH_SECONDS` 配置、当日走势接口与首页自选实时行情行为说明
- [测试] 补充当日走势 snapshot、行情缓存新鲜度、前端共享行情轮询、指标页当日走势与首页自选实时行情测试
- [修复] 当日走势 snapshot 为分钟 K 与实时行情增加短超时预算，上游卡住时返回部分数据；Web 端保留已加载指标图表并显示当日走势失败提示
- [修复] 当日走势过滤开盘前 `close=0` 的无效分钟 K，并在分钟线加载中或暂无有效数据时显示专属状态，避免静默回退成日 K 图表
- [修复] 指标分析日 K 与分钟 K 图表会把实时行情合成到最新交易日或当前分钟桶，随行情轮询同步更新 K线、成交量、MACD/RSI、坐标轴与筹码峰选中日期
- [修复] 指标分析基础行情统一使用最新实时 quote，并兼容 `300274.SZ`/`SZ300274` 等 A 股代码格式；实时 quote 暂不可用时会从所有已加载周期中选取最新、最细粒度 K 线作为统一兜底，避免切换周期或悬停 K 线时基础指标显示不一致
- [新功能] 启用状态的 Web 规则接入实时行情事件触发，行情获取成功后会合成当天判断行并自动计算、记录命中和推送通知
- [改进] 规则最新日扫描在实时 quote 存在时用最新价合成当天 daily 行，让 open/high/low/close、涨跌幅、均线、MACD、RSI 等指标基于实时行情重新计算
- [测试] 补充规则实时行情行合成与启用规则事件触发入库去重的单元测试
- [文档] 更新规则模块设计文档，说明实时事件触发、命中入库推送和按规则/股票/交易日去重的语义
