...
- [新功能] 规则、实测、规则回测和租户飞书通知新增业务租户维度，支持默认租户兼容升级、共享规则只读可克隆、租户独有规则隔离、租户股票池和飞书 Webhook 独立配置，并支持实测/回测一次选择全部租户规则后按租户生成子运行与推送
- [改进] 多租户实测/规则回测新增共享执行规划，逻辑相同的规则按股票并集只扫描一次，并通过租户投影记录各自运行和命中
- [改进] 共享规则命中新增共享运行、共享命中、运行片段和租户命中投影存储，减少多租户共同逻辑重复写入大快照
- [修复] 多租户同步规则回测同样复用共享执行规划，避免 `/api/v1/rules/run-batch` 在多租户请求下重复扫描共同逻辑
- [修复] 多租户 A 股实测 09:30 前预热恢复只刷新 live 缓存、不执行规则扫描，并保持各租户缓存隔离
- [修复] 默认租户飞书通知未配置租户 Webhook 时回退全局配置，避免升级多租户后旧通知配置失效
- [修复] Web 多租户聚合实测/回测运行记录保留子运行租户映射，避免查看命中明细或删除聚合记录时按当前租户请求导致“运行记录不存在”
- [改进] 规则实测固定覆盖全部活跃租户，规则默认全选并保留手动选择；后端 latest 批量运行未传 `tenant_keys` 时也默认扩展为全部活跃租户，回测继续保留租户选择
- [修复] Web 规则实测恢复规则选择器可勾选部分启用规则，同时保持运行租户固定为全部活跃租户
- [修复] 多租户异步实测/回测禁用单租户 running run 复用，统一由共享执行 key 串行去重，避免并发启动相同批次时共同逻辑被拆成私有扫描重复执行
- [修复] 多租户共享执行在非创建者等待已有 running shared run 时不持有进程内锁，并在多进程场景等待数据库完成结果后复用，避免同 key 并发下出现等待闭环或重复计算
- [修复] 多租户共享实测的 `shared:` live cache key 改为按实测 session 稳定，停止实测清理租户 cache 时同步清理对应共享 cache，避免每个新快照重复创建共享缓存并残留
- [修复] 多租户共享规则回测进度写入会过滤保留元数据字段，避免 `completed_count` 重复传参导致全部租户子任务报 `TypeError`
- [修复] 规则运行 API 响应保留实测快路径跳过原因字段，避免前端无法展示 `fast_latest_scan`、`skipped_count` 和 `skip_counts`
- [测试] 补充规则多租户迁移、规则可见性、共享规则只读克隆、运行记录隔离、租户股票池解析和多租户运行聚合测试
- [测试] 补充多租户共享逻辑只执行一次、共享命中投影和逻辑指纹忽略股票池的回归测试
- [测试] 补充共享执行唯一键冲突复用、同步多租户共享执行、预热不触发扫描和默认租户通知回退测试
- [文档] 补充多租户规则工作区、实测/回测一次多租户运行数据链路和飞书通知覆盖边界说明
- [修复] Web 多租户实测/回测轮询保留子运行显式租户请求头，避免全局当前租户覆盖后按错租户读取运行记录并误报“运行记录不存在”
