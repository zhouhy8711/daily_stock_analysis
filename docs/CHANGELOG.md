...
- [修复] Web 规则实测在 09:30 前完成历史缓存预热后会保持实测会话等待开盘，并在 09:30 后自动触发真正实时扫描，避免开盘前点击运行后预热结束就停止
- [修复] A 股规则实测分钟热表成交量会按成交额和价格识别“万手/手/股”口径并统一为手，避免量能条件因热表成交量低估而漏命中
- [修复] 全 A 规则历史回测批量读取日线时按股票限量流式加载，并在大范围任务期间暂停收盘日线归档，避免回测进度接口被大内存缓存和后台归档拖到超时
- [修复] A 股规则实测合成盘中虚拟日 K 时会从本地日线兜底 PE、市值和股本等慢变字段，避免分钟热表 quote 缺少估值字段导致高 PE 分位规则无法命中
- [修复] 规则后台回测启动前会清理服务重启前遗留的 running 任务，并将规则定义指纹纳入复用键，避免规则变更或服务重启后复用旧任务/僵尸任务
- [修复] 所有 A 股规则回测会合并本地 `stock_daily` 中的 A 股代码，避免前端股票索引滞后时漏扫新股或北交所股票
- [修复] Web 回测结果已加载时再次读取运行明细或页面背景刷新超时会保留当前结果并显示可重试提示，避免完成结果上方残留红色超时错误
- [修复] 规则运行历史列表对旧的大结果任务使用快速命中数兜底，避免为展示历史卡片全量解析大 `snapshot_json` 导致 `/api/v1/rules/runs` 超时
- [改进] 规则历史回测仅评估目标日期区间并对未命中条件组短路计算，减少全 A 股严格规则扫描耗时
- [新功能] 规则趋势起涨指标新增实盘观察分、确认信号、追高风险和失效信号，支持只基于当前及历史 K 线构建可实测的起涨规则
- [改进] 规则指标新增市盈TTM 250日分位，并将截图趋势起涨规则收窄为高 PE 分位下的首次突破确认形态，避免全 A 回测泛命中
- [改进] 截图趋势起涨规则补充历史交易日数和前10日突破计数，新增新股/次新股首次20日突破分支，覆盖短历史股票无法计算250日PE分位的场景
- [改进] 补充截图股票泰金新能、鸿仕达的本地名称映射，避免规则命中结果只显示代码
- [改进] Web 规则回测结果每个规则命中分组新增下载按钮，可按当前展示排序导出股票、行业、日期三列 `.xlsx` 文件
- [改进] 每日调度和 Web/API 生命周期新增 A 股除权除息前复权自动刷新后台任务，收盘后按近期事件自检并在确认 stale 时覆盖 `stock_daily`、重建筹码日缓存
- [测试] 补充每日除权除息前复权刷新服务的启停、交易日时间窗、同日去重和失败重试测试
- [文档] 补充每日除权除息前复权自动刷新任务的启用方式、配置项、报告目录和回滚说明
- [chore] 新增 `tools/refresh_qfq_after_corporate_actions.py`，可先 dry-run 审计 A 股除权除息后的前复权历史价差异，确认触发后从本地最早日期重拉 qfq 日线、覆盖 `stock_daily` 并重建筹码日缓存
- [测试] 补充除权除息前复权刷新工具的 dry-run、apply 覆盖和无差异跳过测试，避免因分红事件本身误刷数据库
- [文档] 新增 A 股除权除息后的前复权日线刷新设计与运维说明，记录触发判定、刷新范围、审计报告和回滚方式
- [新功能] 规则指标新增趋势起涨复合信号，支持按多头排列、均线上行、20/60 日突破、放量、MACD 多头和 MA5 偏离过滤筛选趋势股起涨点
- [改进] A 股规则实测对 latest/db-only 的简单最新行情与可按当前价重估的筹码指标规则启用实时 quote 与最新筹码日缓存快路径，并批量读取分钟热表和 `stock_chip_daily`，减少全市场扫描时逐股读取与历史预热耗时
- [改进] A 股规则实测慢路径在预热阶段批量读取本地分钟热表 quote，避免历史指标规则全市场扫描时逐股聚合分钟数据导致耗时放大
- [改进] 规则实测快路径支持用昨日 `stock_chip_daily` 分布、今日分钟热表虚拟日 K 与实时换手率增量重算盘中筹码形态，并在日志中展示行情、筹码和换手率等跳过原因汇总
- [修复] 规则实测快路径限制非 A 股、筹码缺失与陈旧缓存等不安全场景回退或跳过，并在批量 quote 未命中时避免退化为全市场逐股查询
- [修复] Web 规则实测启动请求本地超时时保留同一缓存会话继续重试，停止仍在执行的后台 run 时暂缓清理 live cache，避免误报失败或影响后台任务
- [修复] A 股规则实测运行期间暂停后台全市场实时行情预热，避免实测慢路径与 quote 预热同时拉取全 A 股导致进度轮询超时
- [修复] 分钟热表批量 quote 按市场口径过滤，实测快照覆盖统计只统计 A 股代码与常规交易时段，避免混合市场查询或 09:25/11:31 脏行导致实测误判快照已就绪
- [修复] A 股开盘前规则实测先创建可轮询后台任务再异步预热历史缓存，并在 Web 日志中区分历史缓存预热与实时行情快照，避免预热阶段停在 12% 且误提示快照未就绪
- [修复] Web 规则实测在后台任务已启动后遇到进度轮询或命中明细读取超时时改为可重试警告并保留当前 run，避免把临时读取超时误报为实测失败
- [改进] Web 规则回测和实测的“按行业”股票范围支持多选行业，并按所选行业合并生成股票池
- [修复] 规则实测会把最新分钟热表行情合成当前判断日 K 线并重算当日筹码分布，避免依赖筹码峰指标的盘中实测因当天收盘筹码快照未归档而全量不命中
- [改进] A 股规则实测新增仅实测使用的 live 数据缓存，09:30 前会预热选中规则和股票所需的当天前历史日线，后续轮次持续复用历史、指标与财务事件派生数据，直到用户停止实测时按 session 清理
- [修复] 规则实测和规则回测统一强制使用 `db_only`，只读取 `stock_daily`、`stock_intraday_minute` 和 `stock_chip_daily` 等本地库数据，不再在规则扫描过程中触发 HTTP 行情或财务数据回源
- [改进] A 股日线补数脚本新增 `--derived-only` 并在常规补数后重算 `stock_daily` 派生指标，同时 K 线历史接口透出新增派生指标字段，便于新增规则指标后补齐旧缓存
- [修复] Web 指标分析页日 K 历史指标使用 365 天预热数据计算均线、MACD、RSI 和前 N 日收益，并移除历史 K 线涨跌停价、涨速、委比的估算展示，避免历史指标口径不一致
- [修复] Web 规则回测命中指标弹窗保留命中日之后的本地日 K、成交量和 MACD 指标，避免只画命中日红柱而看不到后续观察天数
- [改进] Web 规则回测和实测命中表在股票列后新增行业列，便于按股票行业快速识别命中结果
- [改进] Web 规则回测和实测命中表的股票、行业和日期表头支持叠加升降序排序，默认按股票 ID 正序、同股按命中时间倒序展示
- [改进] Web 规则回测和实测大结果集按规则分组分页渲染，避免上万条命中记录一次性渲染导致浏览器卡死
- [修复] 规则回测后端在 `all_a_shares` 未显式传入股票列表时会从股票索引解析全量 A 股，避免所有 A 股任务以 0 只股票假完成
- [修复] Web 规则回测结果按实际命中的条件组展示列，避免多条件组规则的 60 日命中行被套用 30 日列后显示为 `--`
- [修复] 分钟热表归档增加非交易日、正式日线覆盖和可疑价格校验，避免周末快照、错位成交额或复权口径混用污染 `stock_daily`
- [修复] efinance 实时行情成交量按成交额和价格自校验为“手”口径，避免“万手”被当作“手”导致量能指标和热表归档失真
- [修复] AkShare A 股历史日线成交量统一由“股”转换为 `stock_daily` 的“手”口径，并在写库层增加成交量兜底归一，避免量能指标被放大 100 倍
- [修复] `stock_daily` 写入层拒绝分钟热表等临时行情源，收盘归档改为刷新正式日线后再清理分钟热表，避免实时快照再次污染回测和指标分析
- [修复] `stock_daily` 写入层跳过成交量和成交额同时为空的正式日线，避免无成交伪 K 线进入指标、筹码和回测链路
- [修复] `stock_daily` 和 `stock_chip_daily` 读写统一 canonical A 股代码，避免 `688498` 与 `688498.SH` 等后缀格式生成重复股票键并影响回测、筹码缓存和指标查询
- [chore] 新增 `tools/repair_stock_daily_quality.py`，支持默认 dry-run 审计并在显式 `--apply` 时备份、隔离和删除 `stock_daily` 中的临时源历史污染行
- [chore] `tools/repair_stock_daily_quality.py` 新增非 canonical A 股代码审计项，便于发现后缀或前缀格式混入正式日线表
- [chore] 新增 `tools/fill_stock_daily_gaps_sina.py`，用于从正式 A 股历史行情源串行补齐污染隔离后的日线缺口，并拒绝不稳定的多线程补数模式
- [新功能] 规则模块新增“净利润断层”财务事件指标，支持扣非净利同比/环比与公告次日跳空、放量、缺口未回补条件组合
- [修复] 净利润断层财务事件接入东财财务分析指标接口并兼容 `REPORT_DATE`、`NOTICE_DATE`、扣非净利及单季度同比/环比字段，避免回测时财务指标为空导致全量不命中
- [改进] 净利润断层财务事件指标会同步写入 `stock_daily` 并更新规则扫描历史缓存，避免新增指标只停留在临时回测上下文
- [修复] Web 回测和实测规则选择器仅展示启用规则，避免已停用规则仍可被选择运行
- [新功能] 规则模块新增“30-60日振幅单峰集中”规则，并补充近30/60日最高最低区间振幅、平均筹码集中度和单峰集中信号指标
- [改进] 缩量后的放倍量规则补充主要筹码峰高低价比条件，并将筹码峰价格带指标同步到筹码日缓存和前后端数据契约
- [改进] `stock_rules` 新增 `is_disable` 隐藏开关，默认关闭；开启后规则列表接口不返回该规则，Web 规则页和回测/实测选择框不再展示
- [chore] 新增根目录 `restart_dsa.sh`，一键按 8000 单端口方式重启本地 Web/API 服务并验证健康接口与前端静态资源
- [修复] Web 规则实测全 A 股扫描复用同一后台任务与实时行情快照，新增规则运行数据预热缓存、进度写库降频、实时行情归档暂停阀、进程内 SQLite 写入队列和进度锁冲突降级，避免重复触发、请求超时和锁冲突
- [修复] Web 指标分析页分时悬停价格改为展示当前 1 分钟最高价，避免与顶部最高价口径不一致
- [修复] Web 指标分析页日 K 在周末或交易日 9:30 前不再展示当天实时虚拟日线，改为锚定最近已开盘交易日
- [修复] Web 指标分析页分时行情按主流行情软件口径统一：开盘价、最高/最低按当日分时会话，涨跌额/涨跌幅按实时价对昨收，均价线按累计成交额/成交量加权，成交额和换手率按当日累计兜底
- [新功能] 规则条件新增“对子数”价格形态操作符，最新价角分两位相同（形如 `.bb`）时命中且无需右侧值
- [改进] 多规则后台回测按股票复用历史行情与指标帧，并在规则未使用筹码指标时跳过筹码计算，减少全市场轻量规则扫描的重复开销
- [新功能] Web 规则模块新增「夹板数」关系，可判断最新价元角分是否呈 `a.ba` 形态
- [修复] Web 指标分析页分时/分钟 K 线刷新时同步按交易日刷新筹码峰指标，并按当前 K 线价格更新获利盘与套牢盘，避免右侧筹码数据停留在旧交易日或旧价格
- [修复] Web 指标分析页分时/分钟顶部估值指标会从同交易日日线缓存兜底，并在 quote 缓存缺失时异步补一次实时行情，同时将分钟成交额标注为本K成交额
- [修复] 实时 quote 快照读取增加市场日期校验，避免程序跨天运行后在第二天开盘前继续复用昨日快照
- [改进] Web 规则实测新增默认开启的精简模式，同一天同一规则和股票仅展示/推送首次命中，关闭后保留逐轮累计结果与原通知去重逻辑
- [修复] 指标分析页分时图在交易日开盘前禁止当天 1 分钟远程回源和脏热表展示，避免把未开始的当天曲线直接画满
- [改进] Web 实测时段门禁改为 A 股交易日 15:00 及以前允许运行，移除 9:30 前和午休拦截
- [改进] 规则实测在实时 quote 快照缺单股时会读取当天分钟热表聚合虚拟日 K 作为本地 fallback，仍保持不远程回源
- [修复] 全 A 股实时 quote 后台预热增加 A 股交易时段门禁，休市后不再按旧快照反复刷新缓存
- [修复] Web 指标分析页 K 线、成交量和 MACD 图右侧保留数据留白，避免最新交易日柱体贴住右侧坐标轴
- [修复] Web 指标分析页在日线缓存停留于上一交易日时追加当天实时日线，并让后端收盘后按当天有效交易日刷新日线缓存
- [修复] 分钟热表收盘归档会校验 A 股交易日，并在写入或补齐当天 `stock_daily` 时回填实时 quote 推导的市值、PE 与股本字段
- [修复] 分钟热表收盘归档在 16:00 后每半小时补检当天 `stock_daily` 与 `stock_chip_daily`，写入前跳过已完成股票，避免单次触发失败后日表缺行或重复写入
- [改进] Web 指标分析页 K 线和成交量涨跌柱改为阳线红色空心、阴线蓝色实心
- [改进] `stock_daily` 增加换手率、市值、PE 与股本字段，日线同步、历史补数和 K 线 API 会同步写入并透出这些指标
- [改进] `tools/backfill_a_share_daily_history.py` 支持 `--refresh-existing` 刷新已有日线，并在写库前补充 quote 推导的估值与股本字段
- [修复] Web 回测指标弹窗在历史命中日优先使用 `stock_daily` 已缓存的市值、PE、量比和换手率，避免历史视图这些核心指标显示为空
- [改进] Web 回测命中指标弹窗改为历史 DB-only 模式，只读取 `stock_daily` 与 `stock_chip_daily`，不再展示实时价格、实时监控或主力持仓面板
- [新功能] 新增 `stock_chip_daily` 筹码峰日缓存，补日线脚本和分析流水线会把筹码快照写入本地库，供回测指标弹窗直接读取
- [改进] `stock_daily` 日线同步路径会同步检查并写入 `stock_chip_daily`，定时任务、历史日线缺口补齐和补数脚本共享 DB-first 的筹码峰缓存语义
- [改进] A 股日线补数脚本支持 `--skip-daily` 只补 `stock_chip_daily`，串行模式增加单股超时保护，并使用 Baostock 换手率计算本地筹码峰快照
- [修复] AkShare 北交所新浪历史行情补齐换手率映射，让本地筹码峰模型可基于北交所日 K 计算快照
- [修复] Web 指标分析扩展指标请求延长超时时间，避免准确筹码数据源响应较慢时右侧筹码峰被前端提前判定为空
- [改进] Web 回测结果列表按股票 ID 正序展示，同一股票内按命中日期倒序排列
...
- [新功能] Web 规则实测在新实时快照产生命中后会将命中规则、股票和条件值推送到所有已配置通知渠道
- [修复] 规则实测在实时行情命中时追加当天判断行，并将 A 股实时成交量按手口径规整，避免命中日停留在上一交易日以及当前量和历史均量 100 倍错位
- [修复] Web 实测页在本地行情或历史缓存未命中时按无数据静默返回，避免展示大段 `history_cache_miss` 跳过原因
- [修复] 指标分析页分时图在当日 09:30 前无分钟数据时保留空图表并持续轮询，首批分钟数据写入后自动渲染价格、成交量、MACD 与筹码面板
- [修复] 指标分析页分时图按目标交易日过滤单日分钟数据，避免开盘前将上一交易日分时误展示为当天波动
- [修复] 指标分析页日线缓存 miss 时自动按默认策略回源补齐，避免全市场实时缓存已有但首次打开日 K 图为空
- [改进] FastAPI/Web 与 schedule 模式新增分钟热表收盘归档后台任务，每 30 分钟扫描并在 16:00 后归档当天数据到 `stock_daily` 后清理热表
- [修复] 股票数据诊断合并同一 A 股的纯代码与交易所后缀代码，避免 `603375` / `603375.SH` 混存时少算历史日线覆盖
- [改进] Web 指标分析页顶部新增当前日期时间时钟，并每秒实时刷新到秒
- [新功能] Web 侧边栏新增实测页，复用回测规则选择与结果展示，支持按实时行情周期触发规则扫描并输出命中结果和执行日志
- [改进] Web 实测页移除回测日期筛选与结果日期列，实时命中结果按精确到秒的执行时间折叠分组并累计展示多轮命中
- [改进] Web 实测页每轮命中分组新增完成时间展示，便于区分触发扫描与结果返回时间
- [修复] Web 实测页执行时间按上海时区生成并展示，避免 ISO UTC 时间被直接截断后比实际触发时间少 8 小时
- [新功能] Web 指标分析页在日K左侧新增分时按钮，基于 1 分钟行情仅展示最新交易日 09:30-15:00 的股价波动
- [修复] Web 指标分析历史行情请求超时时间与后端数据源预算对齐，并将分钟/分时 K 线改为按需加载，减少打开页面时的上游超时
- [修复] 指标分析页日线历史先从 DB 读取且不足时回源补齐入库，当天盘中数据改为通过实时行情接口生成缓存内虚拟日线点，避免 5 月 7 日仍停留在 5 月 6 日
- [修复] Web 指标分析将实时最新价同步到分时、日K、1分、5分、15分、30分和60分已加载周期的最后一个行情点，避免顶部实时价与主图右侧最新价不一致
- [修复] 统一 A 股实时 quote、规则指标和指标 K 线的成交量口径为「手」，避免实测结果与 K 线成交量显示相差 100 倍
- [新功能] 新增 `stock_intraday_minute` 当日分钟热表，后台实时 quote 预热会采样写入分钟 OHLCV，并支持收盘后聚合归档到 `stock_daily`
- [改进] 实时 quote 查询改为优先读取最新预热快照，再读短缓存，最后才按默认策略远程补数；实测使用 `snapshot_only` 避免绕过缓存
- [改进] 指标分析页 quote 与 1m/分时 K 线请求改为优先依赖缓存和分钟热表，并展示 K 线来源与快照时间
- [修复] 指标分析页分时/分钟热表 miss 时按同周期回源预热并写回热表，避免将日 K 多日曲线误展示为分时
- [新功能] 新增股票数据诊断接口和 `tools/check_stock_data_diagnostics.py`，可查看历史日线库、当天分钟热表和实时 quote 缓存覆盖情况
- [改进] Web 系统设置页将实时行情缓存配置入口改为 `REALTIME_QUOTE_CACHE_SECONDS`，字段右侧显示当前实时行情缓存内存占用 MB，并兼容读取旧名 `REALTIME_CACHE_TTL`
- [改进] API 服务启动后新增后台全 A 股实时行情缓存预热，每 1 分钟刷新活跃 A 股 quote 快照，复用 `PREFETCH_REALTIME_QUOTES` 与 `REALTIME_QUOTE_CACHE_SECONDS`
- [改进] 实时行情全市场缓存刷新增加并发合并，复用 `REALTIME_QUOTE_CACHE_SECONDS` 控制的旧缓存，避免缓存过期瞬间重复拉取全市场行情
- [文档] 新增实时行情缓存配置说明，整理后台预热、缓存 TTL、数据源优先级和熔断参数设置方法
- [改进] 实时行情新增 30 秒进程内短缓存，Web 首页自选与 A股所有列表复用已加载报价并保留旧价格等待后台刷新
- [修复] Web 回测执行历史改为展示命中记录数，并在规则历史接口返回展开后的 `event_count`，统一历史卡片、结果标题、Tab 与规则分组的数量口径
- [修复] Web 回测页切换到运行中记录时按当前 run 清空旧命中明细并重新计算结果数量，避免未命中任务显示上一轮结果
- [改进] 多规则回测后端改为规则级并发执行，前端执行日志在运行中持续追加心跳并自动滚动到底部
- [修复] 系统设置中 iFinD 授权用户名按敏感字段使用密码控件，避免 Web 设置页明文展示敏感账号
- [改进] Web 回测页规则历史回测后端改为按股票并发执行，多规则回测保存为单条历史记录，并延长前端请求超时和输出单股执行日志，支持更大股票范围扫描
- [修复] Codex CLI 默认参数移除过期 `general_analytics` feature flag，并兼容过滤旧配置，避免 Web 股票分析调用本地 Codex 时直接失败
- [新功能] 新增 Web 规则页面与 `/api/v1/rules` 规则接口，支持规则 CRUD、条件组配置、历史聚合、连续/频次条件和手动运行结果
- [改进] 规则指标注册表新增解套率、筹码集中度和平均筹码成本，可组合缩量倍量与筹码条件筛选
- [改进] 规则指标注册表补齐指标分析页 K线、成交量、MACD、RSI、筹码峰全部/主力筹码与实时监控指标，并在 Web 规则页按图表类别分组展示
- [改进] 规则指标注册表新增“额外”分类的前5日/前20日累计涨幅指标，按当前判断日前完整交易日复利累计且不包含当前判断日，并放入指标分析页更多指标入口
- [改进] 规则运行改为按股票历史逐日扫描并在运行结果展示命中交易日，同时删除规则前增加确认弹窗
- [修复] 规则回测合并按交易日返回的筹码快照，让解套率、筹码集中度和平均筹码成本可参与历史逐日条件判断
- [改进] Web 规则页将条件偏移字段改为“取值日偏移”，并新增说明提示帮助理解交易日偏移和历史聚合取值
- [改进] 规则运行新增“最新日扫描”和“历史回测”模式，历史回测按股票和交易日展开命中结果并返回每个命中日的指标快照
- [改进] Web 规则页面股票范围新增“所有 A 股”，并在选择自选股时按首页自选监控区同一逻辑填入当前股票代码与名称，选择自定义或所有 A 股时也以右侧清单提取出的股票代码作为扫描范围
- [改进] Web 规则页面股票清单新增最大化查看、代码/名称筛选与单行移除，便于滚动整理大规模 A 股列表
- [改进] Web 规则页移除股票范围与运行结果区，将规则历史回测入口迁移到回测页；回测页新增股票范围选择、执行历史、命中结果表格与指标明细弹窗
- [改进] Web 回测页规则选择改为多选并支持全选/取消，股票范围恢复展开筛选与移除能力，初始状态不再自动加载旧执行历史
- [改进] Web 回测页展开股票列表的行业筛选改为与首页 A 股列表一致的按钮浮层样式，支持行业搜索、多选与清空
- [改进] Web 回测页新增日期范围与按行业扫描入口，规则命中结果按股票、日期和规则条件动态展开左值/右值
- [改进] Web 侧边栏将回测入口调整到规则下方、问股上方
- [改进] Web 回测页运行时在执行历史显示进行中记录和进度，切换页面返回后可继续查看，刷新后会从持久化运行记录恢复历史结果，并新增执行日志 / 运行结果双 Tab
- [改进] Web 回测页命中结果股票列可打开复用的指标分析视图，并自动定位和高亮命中交易日
- [改进] Web 回测页命中结果表将各规则条件的当前值列表头标红，以便和阈值列区分
- [改进] Web 回测页执行历史支持悬停删除单次回测记录，并同步清除持久化命中明细
- [改进] Web 回测页运行结果按规则分组展示，多规则回测可逐条规则展开查看命中股票、日期与对应条件左右值
- [改进] Web 侧边栏品牌文字由 DSA 调整为“必赢”
- [文档] 新增规则模块设计方案，明确规则条件组、指标 key、历史聚合、连续/频次条件与 MVP 边界
...
- [改进] Web 首页“A股所有”列表新增同花顺行业标签列，股票索引生成链路同步写入构建期行业字段
- [改进] Web 首页自选与“A股所有”列表统一展示行业列，并支持按多个行业筛选当前列表，筛选弹层可用中文或拼音首字母快速查找行业
- [改进] Web 规则页股票范围列表显示行业、股票代码和股票名称，并在最大化股票列表中支持按行业类别筛选
...
- [改进] Web 指标分析 K 线、成交量与 MACD 浮窗支持手动拖拽避让遮挡，并在切换日期、周期、窗口或指标轴时恢复默认位置
- [改进] Web 指标分析点击或悬停柱子时不再显示遮挡图表的浮窗，K 线、成交量与 MACD 明细改为在各图表标题栏实时联动展示
- [修复] Web 指标分析 15/30/60 分钟 K 线在数据不足以平移时仍保持时间窗口轴可见
- [改进] Web 指标分析成交量图标题栏去除当前成交量和成交额，保留日期与均量指标
- [改进] Web 指标分析 15/30/60 分钟 K 线扩大历史拉取范围，让时间窗口具备可拖动的历史区间
...
- [改进] Web 指标分析 K 线标题栏补充市值股本、涨跌停价、涨速、主力净流入、换手率、委比、总量与总金额等实时指标
- [改进] Web 指标分析新增顶部核心行情指标区，将收盘价、涨跌幅、高低开、市值、流通、市盈、量比、换手和成交额从平铺指标中独立展示
- [改进] Web 指标分析顶部核心行情指标默认显示最新交易日，并随 K 线悬停、点击固定和键盘移动联动切换
- [改进] Web 指标分析 K 线图内新增左右平移与按选中日期放大缩小的悬浮控制
- [改进] Web 指标分析成交量标题栏集中展示量/额、盘后、MA5/10 均量/均额和换手率，并从 K 线标题栏移除重复量能指标
- [修复] Web 指标分析成交量图按成交量/成交额模式分别显示盘后量与盘后额，并透传腾讯实时行情盘后定价成交字段
- [修复] Web 指标分析核心指标与成交量标题栏共用换手率口径，历史 K 线缺少换手率时按成交量与流通股本推导
- [修复] 实时行情字段补充会继续尝试到腾讯源以获取盘后定价成交量/成交额，成交量标题栏缺少盘后量时按盘后额与最新价反推兜底
- [修复] Web 指标分析核心区市值、流通市值和市盈率持续补齐，市值字段缺失时按股本与选中价格推导
- [修复] Web 指标分析历史 K 线标题栏继续显示股本、涨跌停价、涨速、主力资金和委比，缺少逐日实时字段时按选中 K 线与参考行情推导
- [改进] Web 指标分析页所有核心展示指标新增加号入口，可批量生成同一规则条件组，并补齐规则指标注册表的核心行情、盘后成交、股本、限价、涨速、委比和主力净量指标
- [改进] Web 指标分析页已选规则指标支持右上角浮窗编辑，选中指标的加号切换为减号并可直接移除；K 线时间窗口轴与周期按钮对齐并放大主图，同时将次级 K 线指标收进“更多”浮层，K 线、成交量和 MACD 图支持最大化查看
...
- [改进] Web 首页搜索建议选中股票时改为直接加入自选而不立即分析，并在自选多选模式新增批量取消关注入口
...
- [改进] Web 指标分析新增默认折叠的主力持仓与筹码分布面板，展开后以左侧趋势图和右侧核心指标展示，支持在图表右上角按主力名称下拉切换联动，默认展示所有主力合计
- [改进] Web 指标分析 K 线图区域保持原有图表、趋势摘要和数据说明内容，并新增同风格折叠标题栏
- [新功能] Web 指标分析 K 线图支持日K与 1/5/15/30/60 分钟K切换，后端 `/api/v1/stocks/{code}/history` 同步支持分钟周期参数
- [修复] A 股分钟K在东方财富分钟接口不可用时自动回退到腾讯当日分时接口并本地聚合，避免 Web 指标分析分时切换空图
- [修复] 美股分钟K在 YFinance 限流或东方财富不可用时优先回退 CNBC 多日 OHLCV，并保留 Nasdaq 当日 chart 兜底，避免 BABA 等美股指标分析分时切换空图
- [改进] Web 指标分析主力筹码趋势图新增悬停明细卡，并将右侧指标拆分为随主力下拉联动的主力指标与固定基础指标
- [修复] Web 指标分析主力筹码趋势与 K 线图左右列宽对齐，并修复黄色、紫色图例色条与实际线条颜色不一致的问题
- [改进] 大盘复盘 Prompt 与 Web 时间展示统一使用 24 小时制，避免出现 AM/PM 或上午/下午格式
- [改进] Web 指标分析分钟K新增交易所时区标识与可拖动时间窗口，美股分钟K优先使用 CNBC 多日 OHLCV 以支持回看前一交易日
- [改进] Web 指标分析 1 分钟 K 线打开时每 10 秒静默刷新行情与分钟数据，切换到其他周期或关闭浮窗后自动停止
...
- [改进] Web 自选列表“指标”改为跳转独立指标页，指标页以暗色证券终端图表样式按 K 线、成交量、MACD 分区展示，并在右侧新增筹码峰与实时分单监控兜底展示；桌面侧边栏支持收拢为仅图标导航
- [改进] Web 指标分析页将周期切换移入 K 线图头部，K线、成交量、MACD 支持联动悬浮与右键缩放，筹码峰固定展示数据源日期，实时分单监控每 10 秒静默刷新行情
- [修复] Web 指标分析图表跟随浅色主题切换为浅色交易终端皮肤，避免浅色页面中混入大块暗色图表
- [修复] Web 指标分析去除标题说明文案，周期切换改为后台预热缓存，并避免 A 股/港股休市日分钟K错误显示当天数据
- [改进] Web 指标分析右侧新增筹码峰/实时监控 Tab，默认展示筹码峰，并在筹码图下方补充全部筹码与主力筹码的收盘获利、平均成本、价格区间和集中度指标
- [修复] Web 指标分析在 A 股真实筹码源不可用时不再用历史成交量估算填充筹码指标；若启用本地筹码模型则明确标注来源，避免与同花顺筹码口径混用
