...
| 首頁 | 自選監控 | 首頁預設展示 `STOCK_LIST` 自選股監控列表（未配置時回退最近歷史股票），支援列表多選；自選區「重新分析」會批量分析已選股票，分析進度展示在左側邊欄空白區，點擊股票以浮窗開啟分析報告，報告浮窗可開啟指標分析查看即時行情、K 線、成交量與均線，K 線圖支援日K與 1/5/15/30/60 分鐘K切換，1分鐘K開啟時每 10 秒靜默刷新行情與分鐘數據，分鐘K會標識交易所時區並可拖動時間窗口回看返回區間；A 股分鐘K在東方財富不可用時回退騰訊當日分時並本地聚合，美股分鐘K優先使用 CNBC 多日 OHLCV，在 YFinance 限流或 CNBC 不可用時回退 Nasdaq 當日 chart 價格序列，浮窗支援 Esc 關閉 |
