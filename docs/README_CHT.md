...
| 首頁 | 自選監控 | 首頁預設展示 `STOCK_LIST` 自選股監控列表（未配置時回退最近歷史股票），支援列表多選；自選區「重新分析」會批量分析已選股票，分析進度展示在左側邊欄空白區，點擊股票以浮窗開啟分析報告，報告浮窗可開啟指標分析查看即時行情、K 線、成交量與均線，K 線圖支援日K與 1/5/15/30/60 分鐘K切換，K線、成交量、MACD 與籌碼峰支援點擊固定後用左右方向鍵同步移動，1分鐘K開啟時每 10 秒靜默刷新行情與分鐘數據，分鐘K會標識交易所時區並可拖動時間窗口回看返回區間；A 股分鐘K在東方財富不可用時回退騰訊當日分時並本地聚合，美股分鐘K優先使用 CNBC 多日 OHLCV，在 YFinance 限流或 CNBC 不可用時回退 Nasdaq 當日 chart 價格序列，浮窗支援 Esc 關閉 |
...
| `PUBLIC_FINANCE_NEWS_ENABLED` | 免 Key 公開財經資訊源（預設 `true`，未配置搜索 API Key 時透過 AkShare/東方財富個股新聞、公告和財經快訊為相關資訊兜底） | 可選 |
...
| `REALTIME_QUOTE_CACHE_SECONDS` | 即時行情快照與短快取複用秒數，預設 `30`；`0` 表示停用進程內即時行情快取。舊名 `REALTIME_CACHE_TTL` 仍相容讀取 | 可選 |
