...
4. The Web rule live-test page pushes matched rules, stocks, and condition values to every configured notification channel when a new realtime snapshot hits. Within the same day, if the current "rule + stock" match set is exactly the same as the previous hit set, the app skips the duplicate push until that set changes; the Feishu channel uses `FEISHU_WEBHOOK_URL`.
...
- **Rule Live Test** - The Web live-test page reuses the backtest rule selector and stock scope, periodically scans with realtime quotes, hides backtest date filters, collapses hit results by second-level execution time, shows each cycle's finish time in the group header, and keeps trigger logs updating. New realtime snapshot hits are pushed to every configured notification channel, with same-day duplicate suppression when the "rule + stock" match set is unchanged from the previous hit set.
