...
- **Rule Live Test** - The Web live-test page reuses the backtest rule selector and stock scope, periodically scans with realtime quotes, hides backtest date filters, collapses hit results by second-level execution time, shows each cycle's finish time in the group header, and keeps trigger logs updating.
