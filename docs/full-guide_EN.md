...
| `INDICATOR_INTRADAY_REFRESH_SECONDS` | Polling interval for the Web indicator intraday trend, core quote fields, and Home watchlist live quotes. Default `60`, range `10~3600` seconds; editable in the System settings category. | `60` | Optional |
...
| `INDICATOR_INTRADAY_REFRESH_SECONDS` | Polling interval in seconds for indicator intraday trend, core quotes, and Home watchlist live quotes | `60` |
...
- **Watchlist Monitor Home** - The Home page defaults to a `STOCK_LIST` watchlist monitor (falling back to recent history when unset), supports multi-select, batches selected stocks from the watchlist Reanalyze action, shows analysis progress in the left sidebar action area, supports collapsing the desktop sidebar into icon-only navigation to free workspace width, and opens reports in an Esc-closeable overlay whose sidebar only shows the current stock's history; each row's Indicator action navigates to a standalone indicator page that uses a dark trading-terminal chart style and separates K-line, volume, and MACD-style momentum areas, with real-time quote, daily, same-day intraday trend, and 1/5/15/30/60 minute bars, moving averages, volume ratio, market cap/share capital, limit prices, price speed, entrust ratio, main-force net inflow, total amount, key price levels, a chip-peak panel, and net super-large / large / medium / small order-flow monitoring; the period switcher sits in the K-line chart header, the page prewarms historical periods to avoid full-page loading during switches, A-share/HK minute bars anchor to the latest valid session on closed-market days, K-line/volume/MACD charts share hover state, clicking pins the cursor, left/right arrow keys move the pinned date across K-line, volume, MACD, and chip-peak panels, and the charts support right-click zoom plus only show indicator switches backed by data; core quote fields, the same-day intraday trend, and the Home watchlist silently poll every `INDICATOR_INTRADAY_REFRESH_SECONDS` seconds (default 60), while the “All A-shares” tab remains manual to avoid polling the full market every minute; when live chip or order-flow data is unavailable, the page falls back to historical-volume and price-volume estimates so A-shares and US names such as BABA continue to render.
...
| `/api/v1/stocks/{code}/intraday-snapshot?period=1m` | GET | Query the latest valid trading day's intraday snapshot, including live `quote`, minute K-line `data`, `refresh_interval_seconds`, and partial-failure `errors`; used by the same-day trend tab |
| `/api/v1/stocks/{code}/quote?freshness_seconds=60` / `/api/v1/stocks/quotes` | GET / POST | Query single or batch live quotes; optional `freshness_seconds` caps efinance/akshare full-market cache freshness so frontend polling is not blocked by a longer cache TTL |
