...
4. The Web rule live-test page pushes matched rules, stocks, and condition values to every configured notification channel when a new realtime snapshot hits; the Feishu channel uses `FEISHU_WEBHOOK_URL`.
...
- **Rule Live Test** - The Web live-test page reuses the backtest rule selector and stock scope, periodically scans with realtime quotes, hides backtest date filters, collapses hit results by second-level execution time, shows each cycle's finish time in the group header, and keeps trigger logs updating. New realtime snapshot hits are also pushed to every configured notification channel.
