...
| `PUBLIC_FINANCE_NEWS_ENABLED` | Keyless public finance news source (default `true`; used when no search API key is configured, via AkShare/Eastmoney stock news, announcements, and finance flash feeds) | Optional |
...
| `CODEX_EXEC_AGENT_BACKGROUND_TIMEOUT_SECONDS` | Timeout in seconds for background custom Codex skill Q&A jobs; completed results are written to `skill_out/` and linked in chat history | `7200` | Optional |
...
| `REALTIME_QUOTE_CACHE_SECONDS` | Real-time quote snapshot and short-cache reuse window in seconds; caches per-stock quotes and whole-market DataFrames by time bucket. `0` disables the in-process cache. Legacy `REALTIME_CACHE_TTL` is still read as an alias. | `30` | Optional |
...
The underlying command is shaped like `codex exec --dangerously-bypass-approvals-and-sandbox -m gpt-5.4 "..."`. Regular model calls still isolate user config, project rules, plugins, and tools so analysis prompts are not treated as full Codex agent tasks that browse the web or read/write the repository; when Q&A runs a selected custom Codex skill, it uses `CODEX_EXEC_AGENT_ARGS` instead, preserving local skills, web search, and tools by default. Custom Codex skills run in the background: the page returns a placeholder immediately, the completed Markdown result is written under repo-local `skill_out/`, and the same chat history receives a result link. This mode does not cover Vision/image extraction; image features still need a separate vision-capable API model.
...
For file-based SQLite databases, the app now enables `WAL` and sets `busy_timeout` on connection startup. `save_daily_data()` also uses a batch atomic upsert on `(code, date)`. Rule live-test/backtest runs first preload the daily history required by the selected rules and stock universe. Web live tests use a dedicated live data cache that survives all cycles in the same live-test session and keeps pre-today daily history, base chip distribution, and financial-event derived data until the user stops the live test; each cycle merges the latest intraday hot-table quote into a current-day daily bar and recomputes that day's chip distribution, falling back to profit-ratio repricing only when the live row lacks fields such as turnover rate. Realtime quote warming prioritizes the in-process snapshot; while a large rule live-test is running, low-priority intraday hot-table archival writes are temporarily skipped so they cannot block result persistence. Bulk writes that already use `_run_write_transaction()` now pass through an in-process SQLite writer queue, and async rule-run progress writes skip transient lock conflicts while the scan continues; final result persistence still retries and records explicit failures.
...
- **Watchlist Monitor Home** - The Home page defaults to a `STOCK_LIST` watchlist monitor (falling back to recent history when unset), supports multi-select, batches selected stocks from the watchlist Reanalyze action, shows analysis progress in the left sidebar action area, supports collapsing the desktop sidebar into icon-only navigation to free workspace width, and opens reports in an Esc-closeable overlay whose sidebar only shows the current stock's history; each row's Indicator action now navigates to a standalone indicator page that uses a dark trading-terminal chart style and separates K-line, volume, and MACD-style momentum areas, with real-time quote, daily and 1/5/15/30/60 minute bars, moving averages, volume ratio, market cap/share capital, limit prices, price speed, entrust ratio, main-force net inflow, total amount, key price levels, a chip-peak panel, and net super-large / large / medium / small order-flow monitoring, while the volume header groups post-market volume/amount by active volume-chart mode, MA5/10 average volume, and turnover derived from volume and free float when history omits it; the period switcher sits in the K-line chart header, the page prewarms all periods to avoid full-page loading during switches, A-share/HK minute bars anchor to the latest valid session on closed-market days, K-line/volume/MACD charts share hover state, clicking pins the cursor, left/right arrow keys move the pinned date across K-line, volume, MACD, and chip-peak panels, and the charts support right-click zoom plus only show indicator switches backed by data; the chip-peak panel recalculates for the selected date, order-flow monitoring silently refreshes quote data every 10 seconds, and 1-minute bars additionally refresh minute history while active; when live chip or order-flow data is unavailable, the page falls back to historical-volume and price-volume estimates so A-shares and US names such as BABA continue to render.
- **Rule Live Test** - The Web live-test page reuses the backtest rule selector and stock scope, triggers on A-share trading days until 15:00, and creates a dedicated live data cache for each live-test session. Pre-09:30 runs load the selected rules' pre-today daily history from the local DB into that cache without reading the intraday hot table, evaluating hits, or sending notifications. After 09:30 and during the lunch break, live scans reuse the same history, base chip distribution, and financial-event derived cache, combine it with local intraday hot-table quotes under `db_only`, merge the quote into a current-day daily bar, and recompute current-day chip metrics before evaluating the latest row; stopping the live test clears that session cache. The page supports the "sandwich number" relation for latest prices whose yuan/jiao/fen digits form `a.ba` and the "pair number" relation for cent digits shaped as `.bb`, hides backtest date filters, collapses hit results by second-level execution time, shows each cycle's finish time in the group header, and keeps trigger logs updating. New local hot-table snapshot hits are pushed to every configured notification channel. Compact Mode is on by default, so the same rule and stock only appears and pushes once per day; turning it off restores per-cycle accumulated results and the previous notification dedupe behavior.
- **Quick Analysis** - Trigger analysis via API
- **Real-time Progress** - Analysis task status updates in real-time, supports parallel tasks; the regular stock-analysis path now prefers LiteLLM streaming during the LLM stage and pushes finer-grained `message/progress` updates through task SSE; Web report, history, and market-review timestamps use 24-hour time consistently
