...
- **Rule Live Test** - The Web live-test page reuses the backtest rule selector and stock scope, periodically scans with realtime quotes, supports the "sandwich number" relation for latest prices whose yuan/jiao/fen digits form `a.ba` (same yuan and cent digits with a different middle digit) and the "pair number" relation for cent digits shaped as `.bb`, hides backtest date filters, collapses hit results by second-level execution time, shows each cycle's finish time in the group header, and keeps trigger logs updating. New realtime snapshot hits are pushed to every configured notification channel. Compact Mode is on by default, so the same rule and stock only appears and pushes once per day; turning it off restores per-cycle accumulated results and the previous notification dedupe behavior.
