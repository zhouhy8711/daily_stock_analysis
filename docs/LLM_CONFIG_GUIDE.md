...
问股页的“自定义问询”可以读取本机 `$CODEX_HOME/skills`（默认 `~/.codex/skills`）以及项目内 `.codex/skills`、`.agents/skills` 下的 `SKILL.md`。添加后，前端会把所选 `codex_skill_id` 传给后端，后端会切换到 Codex CLI agent 模式执行该 skill，不再预先调用 DSA 内置行情、K 线、技术指标、筹码或资讯工具，也不再叠加默认交易技能或固定问股模板。
