...
- 旧配置中若包含 `--disable general_analytics`，可直接删除；运行时会兼容过滤该过期 Codex CLI feature flag。
- `CODEX_EXEC_AGENT_ARGS`：问股页选择自定义 Codex skill 时使用的 agent 模式参数，默认只使用 `--dangerously-bypass-approvals-and-sandbox`，保留本机 Codex skill、联网搜索与工具能力，让效果接近直接在 Codex 中调用该 skill。
- `CODEX_EXEC_TIMEOUT_SECONDS`：普通 Codex CLI 补全调用超时时间，默认 180 秒。
- `CODEX_EXEC_AGENT_TIMEOUT_SECONDS`：问股 / Agent 里走 Codex CLI 的链路超时时间，默认 600 秒，适合自定义 skill 联网检索和整理资料。
- `CODEX_EXEC_AGENT_BACKGROUND_TIMEOUT_SECONDS`：问股页自定义 Codex skill 后台任务超时时间，默认 7200 秒；任务完成后 Markdown 结果写入仓库根目录 `skill_out/`，并在同一历史对话追加链接。

Agent 问股在使用 `codex/<model>` 作为主模型时，会先由后端采集行情、K 线、技术指标、筹码与资讯上下文，再通过一次 `codex exec` 生成回答，避免把 Codex CLI 当成多轮函数调用模型而导致对话长时间卡住。

问股页的“自定义问询”可以读取本机 `$CODEX_HOME/skills`（默认 `~/.codex/skills`）以及项目内 `.codex/skills`、`.agents/skills` 下的 `SKILL.md`。添加后，前端会把所选 `codex_skill_id` 传给后端，后端会切换到 Codex CLI agent 模式后台执行该 skill，不再预先调用 DSA 内置行情、K 线、技术指标、筹码或资讯工具，也不再叠加默认交易技能或固定问股模板。页面会立即返回后台执行提示，完成后在 `skill_out/` 生成 Markdown 文件，并把结果链接追加到历史对话。
