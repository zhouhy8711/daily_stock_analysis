...
| Tenants | Rule Workspaces | Switch business tenants in the Web UI; rule histories and match details are tenant-isolated. Live tests always cover all active tenants while keeping the run rules selectable; Web rule backtests can choose all tenants in one run request, then create tenant child runs and Feishu pushes per tenant. Shared rules are read-only and cloneable, while tenant stock pools and Feishu Webhook settings are independent |
| Home | Watchlist Monitor | The Home page defaults to a `STOCK_LIST` watchlist monitor (falling back to recent history when unset), supports multi-select, batches selected stocks from the watchlist Reanalyze action, shows analysis progress in the left sidebar action area, and opens reports in an Esc-closeable overlay; the new "All A-shares" tab lists all A-shares by stock ID, links to the top search box, keeps the same latest-price/change columns as the watchlist with batch real-time quote filling, falls back to Tencent batch quotes when the primary quote source fails, supports ascending/descending sorting by latest price or change in both lists, and lets users add a stock to the watchlist with the plus action; the indicator page provides real-time quote, K-line, volume, moving averages, volume ratio, market cap/share capital, limit prices, price speed, entrust ratio, main-force net inflow, and total amount, while the volume header groups post-market volume/amount by active volume-chart mode, MA5/10 average volume, and turnover derived from volume and free float when history omits it; the right-side indicator area now uses Chip Peak / Realtime Monitor tabs, defaults to Chip Peak, and shows chip-source close-profit, average cost, price range, and concentration details below the chip chart; the K-line chart can switch between daily bars and 1/5/15/30/60 minute bars, the K-line/volume/MACD charts share hover state, clicking pins the cursor, and left/right arrow keys move that pinned date across the K-line, volume, and MACD panels; it silently refreshes quote and minute history every 10 seconds while 1-minute bars are active, labels the exchange timezone, and lets users drag the time window across the returned range; A-share minute bars fall back to Tencent same-day minute data when Eastmoney is unavailable, while US minute bars prefer CNBC multi-day OHLCV and fall back to Nasdaq same-day chart prices when YFinance is rate-limited or CNBC is unavailable; major holder and chip distribution details are collapsed by default and expand into a left trend chart with hover details plus right-aligned metrics, with a top-right holder-name dropdown that drives holder-scoped metrics while fixed market metrics remain in a separate baseline section; the K-line chart area keeps its original chart, trend summary, and data note while adding a matching collapsible header; Chip Peak prefers iFinD/Tushare real per-price chip details, falls back to a turnover-based local chip model labelled `local_chip_model`, switches by selected K-line date when local snapshots are available, and still shows unavailable when both are missing |
...
> Business tenants do not change the single-admin login model. Upgrades create a `default` tenant, move existing rules into the shared rule library, and attach existing rule runs/matches to the default tenant. Live tests always run across all active tenants with all enabled rules selected by default, but the run rules can be adjusted before starting; Web rule backtests can run the current tenant or all tenants in one request. Child histories and Feishu pushes are kept per tenant. v1 tenant-scopes rules, live tests, Web rule backtests, and Feishu Webhook settings; AI analysis-history backtests under `/api/v1/backtest/*` remain global.
...
| `IFIND_USERNAME` / `IFIND_PASSWORD` | Licensed Tonghuashun iFinD account. When configured, A-share chip distribution tries iFinD before Tushare; exact chip indicator codes must be confirmed in the iFinD data browser. | Optional |
| `IFIND_CHIP_SUMMARY_INDICATORS` / `IFIND_CHIP_DISTRIBUTION_INDICATORS` | iFinD chip summary and per-price distribution indicator codes. The per-price result must include price and percent columns, otherwise the system falls back to Tushare, the local turnover-based chip model, or AkShare summary data. | Optional |
