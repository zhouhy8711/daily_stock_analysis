...
const fieldTitleMap: Record<string, string> = {
  ...
  PUBLIC_FINANCE_NEWS_ENABLED: '免 Key 公开财经资讯源',
  ...
  CODEX_EXEC_ENABLED: '启用 Codex CLI',
  CODEX_EXEC_MODEL: 'Codex CLI 模型',
  CODEX_EXEC_COMMAND: 'Codex CLI 命令',
  CODEX_EXEC_ARGS: 'Codex CLI 参数',
  CODEX_EXEC_TIMEOUT_SECONDS: 'Codex CLI 超时（秒）',
  CODEX_EXEC_AGENT_TIMEOUT_SECONDS: 'Codex Agent 前台超时（秒）',
  CODEX_EXEC_AGENT_BACKGROUND_TIMEOUT_SECONDS: 'Codex Agent 后台超时（秒）',
  ...
  LOG_LEVEL: '日志级别',
  WEBUI_PORT: 'WebUI 端口',
  REALTIME_QUOTE_CACHE_SECONDS: '实时行情快照缓存时间（秒）',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  PUBLIC_FINANCE_NEWS_ENABLED: '默认开启；未配置搜索 Key 时，通过 AkShare/东方财富个股新闻、公告和财经快讯为相关资讯兜底。',
  ...
  CODEX_EXEC_ENABLED: '启用后可用本机已登录的 codex exec 作为无 API Key 的模型运行时。',
  CODEX_EXEC_MODEL: '传给 codex exec -m 的模型名；启用后若主模型为空，会自动使用 codex/<该模型>。',
  CODEX_EXEC_COMMAND: 'Codex CLI 可执行命令。默认 codex；只有命令不在 PATH 时才需要改。',
  CODEX_EXEC_ARGS: '传给 codex exec 的附加参数。默认使用跳过审批与沙箱，并隔离用户配置、项目规则、插件和工具能力。',
  CODEX_EXEC_TIMEOUT_SECONDS: '单次 Codex CLI 模型调用最长等待秒数，默认 180 秒。',
  ...
  REALTIME_QUOTE_CACHE_SECONDS: '实时行情快照与短缓存复用时间，默认 30 秒；设置为 0 可禁用进程内实时行情缓存。',
  CODEX_EXEC_AGENT_TIMEOUT_SECONDS: '问股页同步等待 Codex CLI Agent 的时间预算。自定义 Codex skill 会优先转入后台执行，不再长期占用页面请求。',
  CODEX_EXEC_AGENT_BACKGROUND_TIMEOUT_SECONDS: '自定义 Codex skill 后台任务的最长运行时间；超时后会在 skill_out 结果文件中记录失败原因。',
