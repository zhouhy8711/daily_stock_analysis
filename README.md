...
| 首页 | 自选监控 | 首页默认展示 `STOCK_LIST` 自选股监控列表（无配置时回退最近历史股票），支持列表多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开分析报告，报告浮窗可打开指标分析查看实时行情、K 线、成交量与均线，浮窗支持 Esc 关闭 |
...
在首页历史记录中选择一条分析记录后，点击「完整分析报告」按钮可在右侧抽屉中查看与推送通知格式一致的完整 Markdown 分析报告，包含舆情情报、核心结论、数据透视、作战计划等完整内容。报告浮窗内的「指标分析」会打开可关闭的技术指标浮窗，展示实时行情、历史 K 线、成交量、MA5/MA10/MA20、量比、近 20 日支撑/压力等数据。
