...
在首页历史记录中选择一条分析记录后，点击「完整分析报告」按钮可在右侧抽屉中查看与推送通知格式一致的完整 Markdown 分析报告，包含舆情情报、核心结论、数据透视、作战计划等完整内容。自选列表的「指标」按钮会跳转独立指标页，以暗色证券终端图表样式展示实时行情、历史 K 线、成交量、MA5/MA10/MA20/MA30/MA60、量比、近 20 日支撑/压力、MACD、RSI；右侧指标区以筹码峰/实时监控 Tab 切换，默认展示筹码峰，并在筹码图下方展示筹码源返回的收盘获利、平均成本、90%/70%价格区间和集中度，实时监控切换后展示净特大单/净大单/净中单/净小单；K 线图头部可在日K与 1/5/15/30/60 分钟K之间切换，页面会后台预热各周期以避免切换时整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 会联动悬浮日期，点击固定后可用左右方向键同步移动 K线、成交量和 MACD，筹码峰优先展示数据源返回日期的真实逐价位分布，缺失时使用含换手率或可推导换手率日 K 的本地筹码模型并标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，三张图支持右键放大/缩小，成交量与动能区仅展示有数据的指标切换按钮；实时分单监控每 10 秒静默刷新行情，1分钟K打开时额外刷新分钟数据，分钟K通过 `/api/v1/stocks/{code}/history?period=5m` 等周期参数按需拉取，并在图表中标识交易所时区、支持拖动时间窗口回看返回区间；A 股在东方财富分钟接口不可用时会回退腾讯当日分时并本地聚合，美股优先使用 CNBC 多日分钟 OHLCV，在 YFinance 限流或 CNBC 不可用时继续回退 Nasdaq 当日 chart 价格序列；筹码峰不再展示历史成交量或股东持仓估算值，真实逐价位筹码明细与本地模型均缺失时显示不可用，分单接口不可用时实时监控仍会标记为基于价量关系估算。
