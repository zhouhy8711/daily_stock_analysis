...
| 首页 | 自选监控 | 首页默认展示 `STOCK_LIST` 自选股监控列表（无配置时回退最近历史股票），支持列表多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开分析报告；“A股所有”Tab 按股票 ID 展示全量 A 股并联动顶部搜索，保留与自选一致的最新价/涨跌幅列并通过批量实时行情填充，行情源失败时会回退腾讯批量接口，两处列表均支持按最新价或涨跌幅升降序排序，可用加号一键加入自选；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示日K、当日走势与 1/5/15/30/60 分钟K、均线、量比、市值股本、涨跌停价、涨速、委比、主力资金、关键价位，成交量栏集中展示量、盘后（成交量模式显示盘后量，成交额模式显示盘后额）、MA5/10 均量和换手率（历史缺失时按成交量与流通股本推导），右侧以筹码峰/实时监控 Tab 切换，默认展示筹码峰并在图下方提供筹码源返回的收盘获利、平均成本、价格区间和集中度；周期按钮位于 K 线图头部，三张图支持联动悬浮、点击固定后用左右方向键同步移动、右键缩放和按可用数据切换指标，指标页核心行情、当日走势和首页自选列表按 `INDICATOR_INTRADAY_REFRESH_SECONDS`（默认 60 秒）静默刷新，“当日走势”会同步刷新分钟 K、成交量与 MACD/RSI；“A股所有”Tab 仍保持手动加载/刷新，避免每分钟请求全市场；筹码峰优先展示 iFinD/Tushare 真实逐价位筹码明细，缺失时尝试使用含换手率或可推导换手率日 K 的本地筹码模型并在来源标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，两者均不可用时显示不可用；分单源不可用时实时监控仍会用历史成交量和价量关系估算以保持连续展示 |
...
> Web 工作台已完成一轮 UI 升级：新增完整浅色主题，并支持浅色 / 深色主题一键切换；首页、问股、回测、持仓、设置页改为共享同一套设计 token、输入表面、状态反馈和抽屉/滚动语义，桌面侧边栏支持收拢为仅图标导航，日常使用与移动端浏览更连贯。
...
| `INDICATOR_INTRADAY_REFRESH_SECONDS` | Web 指标页当日走势、核心行情与首页自选实时行情轮询周期，默认 `60`，范围 `10~3600` 秒；也可在系统设置 `System` 分类中修改 | 可选 |
...
在首页历史记录中选择一条分析记录后，点击「完整分析报告」按钮可在右侧抽屉中查看与推送通知格式一致的完整 Markdown 分析报告，包含舆情情报、核心结论、数据透视、作战计划等完整内容。自选列表的「指标」按钮会跳转独立指标页，以暗色证券终端图表样式展示实时行情、历史 K 线、当日走势、成交量、MA5/MA10/MA20/MA30/MA60、量比、市值股本、涨跌停价、涨速、主力净量/净流入、委比、总金额、近 20 日支撑/压力、MACD、RSI；成交量标题栏集中展示量、盘后（成交量模式显示盘后量，成交额模式显示盘后额）、MA5/10 均量和换手率（历史缺失时按成交量与流通股本推导）；右侧指标区以筹码峰/实时监控 Tab 切换，默认展示筹码峰，并在筹码图下方展示筹码源返回的收盘获利、平均成本、90%/70%价格区间和集中度，实时监控切换后展示净特大单/净大单/净中单/净小单；K 线图头部可在日K、当日走势与 1/5/15/30/60 分钟K之间切换，页面会后台预热各周期以避免切换时整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 会联动悬浮日期，点击固定后可用左右方向键同步移动 K线、成交量和 MACD，筹码峰优先展示数据源返回日期的真实逐价位分布，缺失时使用含换手率或可推导换手率日 K 的本地筹码模型并标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，三张图支持右键放大/缩小，成交量与动能区仅展示有数据的指标切换按钮；指标页核心行情、当日走势和首页自选列表按 `INDICATOR_INTRADAY_REFRESH_SECONDS`（默认 60 秒）静默刷新，“当日走势”通过 `/api/v1/stocks/{code}/intraday-snapshot` 拉取最近有效交易日分钟 K 与实时 quote，普通分钟K仍可通过 `/api/v1/stocks/{code}/history?period=5m` 等周期参数按需拉取；“A股所有”Tab 不自动轮询全市场，仅在手动进入或刷新时批量获取行情；A 股在东方财富分钟接口不可用时会回退腾讯当日分时并本地聚合，美股优先使用 CNBC 多日分钟 OHLCV，在 YFinance 限流或 CNBC 不可用时继续回退 Nasdaq 当日 chart 价格序列；筹码峰不再展示历史成交量或股东持仓估算值，真实逐价位筹码明细与本地模型均缺失时显示不可用，分单接口不可用时实时监控仍会标记为基于价量关系估算。
