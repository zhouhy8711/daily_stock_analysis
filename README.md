...
| 首页 | 自选监控 | 首页默认展示 `STOCK_LIST` 自选股监控列表（无配置时回退最近历史股票），支持列表多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开分析报告，报告浮窗可打开指标分析查看实时行情、K 线、成交量、均线；K 线图支持日K与 1/5/15/30/60 分钟K切换，1分钟K打开时每 10 秒静默刷新行情与分钟数据，分钟K标识交易所时区并可拖动时间窗口回看返回区间；主力持仓与筹码分布默认折叠，展开后以左侧趋势图和右侧指标展示，趋势图支持悬停明细，并可在右上角按主力名称下拉切换联动，右侧拆分主力指标和基础指标；K 线图区域保持原内容并支持同风格折叠；真实筹码源不可用时会用历史成交量估算成本分布，浮窗支持 Esc 关闭 |
...
在首页历史记录中选择一条分析记录后，点击「完整分析报告」按钮可在右侧抽屉中查看与推送通知格式一致的完整 Markdown 分析报告，包含舆情情报、核心结论、数据透视、作战计划等完整内容。报告浮窗内的「指标分析」会打开可关闭的技术指标浮窗，展示实时行情、历史 K 线、成交量、MA5/MA10/MA20、量比、近 20 日支撑/压力等数据；K 线图可在日K与 1/5/15/30/60 分钟K之间切换，1分钟K打开时每 10 秒静默刷新行情与分钟数据，分钟K通过 `/api/v1/stocks/{code}/history?period=5m` 等周期参数按需拉取，并在图表中标识交易所时区、支持拖动时间窗口回看返回区间；A 股在东方财富分钟接口不可用时会回退腾讯当日分时并本地聚合，美股优先使用 CNBC 多日分钟 OHLCV，在 YFinance 限流或 CNBC 不可用时继续回退 Nasdaq 当日 chart 价格序列；主力持仓与筹码分布默认折叠，展开后左侧展示收盘价、筹码成本、主力动能与获利盘趋势并支持悬停明细，图表右上角提供主力名称下拉框，默认展示所有主力合计，选择单个主力后图表和右侧主力指标按该主力口径联动，基础指标保持市场口径独立展示；下方 K 线图区域保持原有 K 线、趋势摘要和数据说明内容，并支持同风格折叠；当真实筹码接口不可用时，筹码成本分布会标记为基于历史成交量估算。
