...
| 界面 | 双主题工作台 | Web 工作台现支持全新浅色主题与深色主题切换，首页 / 问股 / 实测 / 回测 / 持仓 / 设置统一升级为同一套视觉与交互体系 |
| 租户 | 规则工作区 | Web 可切换业务租户；规则列表、规则运行历史与命中明细按租户隔离；实测固定覆盖全部活跃租户且可选择本次运行规则，规则回测可一次选择全部租户规则并按租户生成子运行和飞书推送，逻辑相同的多租户规则会共享一次物理扫描；共享规则所有租户可见但只读，可克隆为当前租户独有规则；租户股票池与飞书 Webhook 独立配置 |
| 首页 | 自选监控 | 首页默认展示 `STOCK_LIST` 自选股监控列表（无配置时回退最近历史股票），支持列表多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开分析报告；“A股所有”Tab 按股票 ID 展示全量 A 股并联动顶部搜索，保留与自选一致的最新价/涨跌幅列并通过批量实时行情填充，行情源失败时会回退腾讯批量接口，两处列表均支持按最新价或涨跌幅升降序排序，可用加号一键加入自选；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示日K与 1/5/15/30/60 分钟K、均线、量比、市值股本、涨跌停价、涨速、委比、主力资金、关键价位，成交量栏集中展示量、盘后（成交量模式显示盘后量，成交额模式显示盘后额）、MA5/10 均量和换手率（历史缺失时按成交量与流通股本推导），右侧以筹码峰/实时监控 Tab 切换，默认展示筹码峰并在图下方提供筹码源返回的收盘获利、平均成本、价格区间和集中度；周期按钮位于 K 线图头部，三张图支持联动悬浮、点击固定后用左右方向键同步移动、右键缩放和按可用数据切换指标，实时分单监控每 10 秒静默刷新行情，1分钟K额外刷新分钟数据；筹码峰优先展示 iFinD/Tushare 真实逐价位筹码明细，缺失时尝试使用含换手率或可推导换手率日 K 的本地筹码模型并在来源标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，两者均不可用时显示不可用；分单源不可用时实时监控仍会用历史成交量和价量关系估算以保持连续展示 |
...
| 历史记录 | 批量管理 | 支持多选、全选及批量删除历史分析记录，优化管理效率与 UI/UX 体验；报告浮窗左侧仅展示当前股票的历史记录 |
...
> Web 工作台已完成一轮 UI 升级：新增完整浅色主题，并支持浅色 / 深色主题一键切换；首页、问股、实测、回测、持仓、设置页改为共享同一套设计 token、输入表面、状态反馈和抽屉/滚动语义，桌面侧边栏支持收拢为仅图标导航，日常使用与移动端浏览更连贯。
...
> 业务租户不会改变单管理员登录体系。升级后会自动创建 `default` 租户，历史规则进入共享规则库，历史规则运行与命中归属默认租户；新租户默认股票池为空，需要在 Web 设置页维护。实测运行时固定覆盖全部活跃租户，规则默认全选但可手动调整；规则回测运行时可选择当前租户或全部租户。一次启动后按租户子运行分别记录历史和推送飞书。第一版仅租户化规则、实测、规则回测和飞书 Webhook，AI 分析记录回测 `/api/v1/backtest/*` 仍保持全局。
...
| `MAX_WORKERS` | 异步分析任务队列与 Web 规则历史回测的并发线程数（默认 `3`）；保存后队列空闲时自动应用，繁忙时延后生效 | 可选 |
...
| `PUBLIC_FINANCE_NEWS_ENABLED` | 免 Key 公开财经资讯源（默认 `true`，未配置搜索 API Key 时通过 AkShare/东方财富个股新闻、公告和财经快讯为相关资讯兜底） | 可选 |
...
| `REALTIME_QUOTE_CACHE_SECONDS` | 实时行情快照与短缓存复用秒数，默认 `30`；`0` 表示禁用进程内实时行情缓存。旧名 `REALTIME_CACHE_TTL` 仍兼容读取 | 可选 |
...
| `IFIND_CHIP_SUMMARY_INDICATORS` / `IFIND_CHIP_DISTRIBUTION_INDICATORS` | iFinD 筹码汇总与逐价位分布指标代码；逐价位结果需包含价格与占比列，缺失时自动降级到 Tushare、本地换手率筹码模型或 AkShare 摘要 | 可选 |
...
- **主题切换可持久化**：侧边栏内可随时切换浅色 / 深色主题，页面刷新后仍保留用户选择；桌面端侧边栏可收拢为仅图标模式，释放右侧工作区空间。
- **核心页面统一收口**：首页、问股、实测、回测、持仓、设置共用同一套视觉 token、状态原语和表面组件，减少“每页一个风格”的割裂感。
...
在首页历史记录中选择一条分析记录后，点击「完整分析报告」按钮可在右侧抽屉中查看与推送通知格式一致的完整 Markdown 分析报告，包含舆情情报、核心结论、数据透视、作战计划等完整内容。自选列表的「指标」按钮会跳转独立指标页，以暗色证券终端图表样式展示实时行情、历史 K 线、成交量、MA5/MA10/MA20/MA30/MA60、量比、市值股本、涨跌停价、涨速、主力净量/净流入、委比、总金额、近 20 日支撑/压力、MACD、RSI；成交量标题栏集中展示量、盘后（成交量模式显示盘后量，成交额模式显示盘后额）、MA5/10 均量和换手率（历史缺失时按成交量与流通股本推导）；右侧指标区以筹码峰/实时监控 Tab 切换，默认展示筹码峰，并在筹码图下方展示筹码源返回的收盘获利、平均成本、90%/70%价格区间和集中度，实时监控切换后展示净特大单/净大单/净中单/净小单；K 线图头部可在日K与 1/5/15/30/60 分钟K之间切换，页面会后台预热各周期以避免切换时整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 会联动悬浮日期，点击固定后可用左右方向键同步移动 K线、成交量和 MACD，筹码峰优先展示数据源返回日期的真实逐价位分布，缺失时使用含换手率或可推导换手率日 K 的本地筹码模型并标注 `local_chip_model`，点击或键盘移动 K 线日期时随本地模型快照同步切换，三张图支持右键放大/缩小，成交量与动能区仅展示有数据的指标切换按钮；实时分单监控每 10 秒静默刷新行情，1分钟K打开时额外刷新分钟数据，分钟K通过 `/api/v1/stocks/{code}/history?period=5m` 等周期参数按需拉取，并在图表中标识交易所时区、支持拖动时间窗口回看返回区间；A 股在东方财富分钟接口不可用时会回退腾讯当日分时并本地聚合，美股优先使用 CNBC 多日分钟 OHLCV，在 YFinance 限流或 CNBC 不可用时继续回退 Nasdaq 当日 chart 价格序列；筹码峰不再展示历史成交量或股东持仓估算值，真实逐价位筹码明细与本地模型均缺失时显示不可用，分单接口不可用时实时监控仍会标记为基于价量关系估算。
