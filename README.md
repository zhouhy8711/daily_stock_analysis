...
| 首页 | 自选监控 | 首页默认展示 `STOCK_LIST` 自选股监控列表（无配置时回退最近历史股票），支持列表多选；自选区“重新分析”会批量分析已选股票，分析进度展示在左侧边栏空白区，点击股票以浮窗打开分析报告；“A股所有”Tab 按股票 ID 展示全量 A 股并联动顶部搜索，保留与自选一致的最新价/涨跌幅列并通过批量实时行情填充，行情源失败时会回退腾讯批量接口，两处列表均支持按最新价或涨跌幅升降序排序，可用加号一键加入自选；列表行级「指标」会跳转独立指标页，以暗色证券终端图表样式按 K 线、成交量、MACD 等三段展示日K与 1/5/15/30/60 分钟K、均线、量比、关键价位、筹码峰和实时分单监控；周期按钮位于 K 线图头部，三张图支持联动悬浮、点击固定后用左右方向键同步移动、右键缩放和按可用数据切换指标，实时分单监控每 10 秒静默刷新行情，1分钟K额外刷新分钟数据；真实筹码或分单源不可用时会用历史成交量和价量关系估算，保证 A 股与 BABA 等美股也能连续展示 |
...
在首页历史记录中选择一条分析记录后，点击「完整分析报告」按钮可在右侧抽屉中查看与推送通知格式一致的完整 Markdown 分析报告，包含舆情情报、核心结论、数据透视、作战计划等完整内容。自选列表的「指标」按钮会跳转独立指标页，以暗色证券终端图表样式展示实时行情、历史 K 线、成交量、MA5/MA10/MA20/MA30/MA60、量比、近 20 日支撑/压力、MACD、RSI、筹码峰和净特大单/净大单/净中单/净小单监控；K 线图头部可在日K与 1/5/15/30/60 分钟K之间切换，页面会后台预热各周期以避免切换时整页加载，A 股/港股休市日分钟K会锚定最近有效交易日，K线、成交量和 MACD 会联动悬浮日期，点击固定后可用左右方向键同步移动 K线、成交量、MACD 和筹码峰，筹码峰随选中日期重算，三张图支持右键放大/缩小，成交量与动能区仅展示有数据的指标切换按钮；实时分单监控每 10 秒静默刷新行情，1分钟K打开时额外刷新分钟数据，分钟K通过 `/api/v1/stocks/{code}/history?period=5m` 等周期参数按需拉取，并在图表中标识交易所时区、支持拖动时间窗口回看返回区间；A 股在东方财富分钟接口不可用时会回退腾讯当日分时并本地聚合，美股优先使用 CNBC 多日分钟 OHLCV，在 YFinance 限流或 CNBC 不可用时继续回退 Nasdaq 当日 chart 价格序列；当真实筹码或分单接口不可用时，页面会标记为基于历史成交量和价量关系估算。
